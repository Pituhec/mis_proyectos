          ...
          return (
            <Link
              ...
              className={`flex-1 flex flex-col items-center justify-center gap-1 py-3 min-h-[60px] transition-colors ${
                active ? "text-nexa-600" : "text-slate-400 hover:text-slate-600"
              }`}
            >
              {icon}
              <span className={`text-[11px] font-medium leading-none ${active ? "text-nexa-600" : ""}`}>
                {label}
              </span>
              {active && <span className="absolute bottom-0 w-8 h-0.5 bg-nexa-600 rounded-full" />}
            </Link>
          );
        })}
      </div>
